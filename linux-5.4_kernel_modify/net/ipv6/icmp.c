...
int send_icmpv6_block_confirmation_ip(struct sk_buff *buff_skb, u8 code, uint16_t block_number, uint16_t packet_number, struct in6_addr *saddr, struct in6_addr *daddr) {
    ...
    icmp6h->icmp6_cksum = 0;
	icmp6h->icmp6_dataun.u_block_msg.block_number = htons(block_number);
    icmp6h->icmp6_dataun.u_block_msg.packet_number = htons(packet_number);
	...
	my_dev = dev_get_by_name(&init_net, buff_skb->dev->name);
    if (my_dev) {
	...
	printk(KERN_INFO "dev->name = %s %s %d-(%d)\n",
	    skb->dev->name, skb->dev->dev_addr, skb->dev->addr_len, skb->dev->flags);
	...
	printk(KERN_INFO "Send successfully!\n");
    return res;
}

EXPORT_SYMBOL(send_icmpv6_block_confirmation_ip);

// handle block confirmation
void handle_icmpv6_block_confirmation(struct sk_buff *skb)
{
	struct icmp6hdr *icmp6h;
	...
	icmp6h = icmp6_hdr(skb);
	block_number = ntohs(icmp6h->icmp6_dataun.u_block_msg.block_number);
	packet_number = ntohs(icmp6h->icmp6_dataun.u_block_msg.packet_number);

	printk(KERN_INFO "Received ICMP block number: %u	packet number: %u	code: %u\n", block_number, packet_number, icmp6h->icmp6_code);

	switch (icmp6h->icmp6_code) {
	case 1:
		if (!delete_packet(block_number, packet_number)) {
			// delete successfully
			printk(KERN_INFO "Delete Packet successfully-----Block number: %u	Packet number:%u\n", block_number, packet_number);
		}else {
			printk(KERN_INFO "Fail to Delete the packet-----Block number: %u	Packet number:%u\n", block_number, packet_number);
		}
		if (!send_icmpv6_block_confirmation_ip(skb, 5, block_number, packet_number, &ipv6_hdr(skb)->daddr, &ipv6_hdr(skb)->saddr)) {
			// send successfully
            printk(KERN_INFO "Send block confirmation ACK successfully\n");
        } else {
	...
	case 3:
	case 4:
		printk(KERN_INFO "This feature is not supported at the moment\n");
		break;
	case 5:
		printk(KERN_INFO "Confirm ACK successfully\n");
		break;
	default:
		printk(KERN_WARNING "Unknown code: %u\n", icmp6h->icmp6_code);
		break;
	}
}
...
static int icmpv6_rcv(struct sk_buff *skb)
{
	...
	case 220:
		printk(KERN_INFO "Match 220 successfully!");
		handle_icmpv6_block_confirmation(skb);
		break;
